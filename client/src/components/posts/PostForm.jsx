...
export default function PostForm({ onSuccess, onCancel }) {
  ...
  const textColor   = 'var(--text-2)';
  const subColor    = 'var(--text-4)';
  const inputBg     = 'var(--surface-1)';
  const inputBorder = '1.5px solid var(--surface-1-border)';
  const btnBorder   = '1px solid var(--surface-1-border)';
  const btnColor    = 'var(--text-3)';
    ...
    <div style={{ padding:'26px 28px' }}>
      <div style={{ display:'flex', alignItems:'center', gap:'12px', marginBottom:'20px' }}>
        ...
        <button onClick={onCancel}
          style={{ marginLeft:'auto', width:'34px', height:'34px', borderRadius:'10px', background:'var(--surface-1)', border:btnBorder, color:subColor, cursor:'pointer', fontSize:'18px', display:'flex', alignItems:'center', justifyContent:'center', transition:'all .2s', fontFamily:'inherit' }}
      ...
      <form onSubmit={submit}>
        <textarea value={content} onChange={e=>setContent(e.target.value)} required rows={5}
          ...
          onFocus={e=>{ e.target.style.borderColor='rgba(124,58,237,0.55)'; e.target.style.boxShadow='0 0 0 4px rgba(124,58,237,0.12)'; e.target.style.background='var(--surface-2)'; }}
          onBlur={e=>{  e.target.style.borderColor='var(--surface-1-border)'; e.target.style.boxShadow='none'; e.target.style.background=inputBg; }}
        />
        ...
        <div style={{ display:'flex', justifyContent:'flex-end', gap:'10px', paddingTop:'16px', borderTop:'1px solid var(--divider)' }}>
      ...
      <style>{`textarea::placeholder{color:var(--text-5);} @keyframes spin{to{transform:rotate(360deg);}}`}</style>
    </div>
  );
}
