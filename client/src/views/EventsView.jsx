...
export default function EventsView() {
  ...
  return (
    <div style={{ minHeight:'100vh', position:'relative', color:'var(--text-1)' }}>
      ...
      {/* FONDO */}
      <div style={{ position:'fixed', inset:0, zIndex:0, background:'var(--bg-events)' }}/>
      ...
      <div style={{ marginLeft:'90px', padding:'40px 48px 60px', position:'relative', zIndex:1 }}>
        ...
        <div style={{ marginBottom:'36px' }}>
          ...
          <h1 style={{ fontSize:'64px', fontWeight:900, lineHeight:0.92, letterSpacing:'-3px' }}>
            ...
            <span style={{ color:'var(--text-1)' }}>tos</span>
          </h1>
        </div>
        ...
        {featured && (
          ...
          >
            ...
            <div style={{ padding:'24px 28px' }}>
              <p style={{ fontSize:'16px', color:'var(--text-3)', lineHeight:1.65, marginBottom:'20px', maxWidth:'680px' }}>{featured.desc}</p>
                ...
                <div style={{ display:'flex', gap:'24px', flexWrap:'wrap' }}>
                  {[
                    ...
                    <div key={txt} style={{ display:'flex', alignItems:'center', gap:'7px' }}>
                      ...
                      <span style={{ fontSize:'14px', color:'var(--text-3)', fontWeight:500 }}>{txt}</span>
                    </div>
                  ))}
                </div>
                <button onClick={()=>toggleRSVP(featured.id)}
                  style={{ padding:'13px 28px', borderRadius:'16px', border:'none', color: featured.rsvp ? 'var(--text-2)' : '#fff', fontSize:'15px', fontWeight:700, cursor:'pointer', transition:'all .3s', fontFamily:'inherit',
                    background: featured.rsvp ? 'var(--surface-2)' : `linear-gradient(135deg,${featured.color},#4F46E5)`,
              ...
              >
                ...
                <div style={{ padding:'22px 24px' }}>
                  ...
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'14px' }}>
                    ...
                    <span style={{ fontSize:'13px', color:'var(--text-4)', fontWeight:600 }}>{daysLeft(ev.date)}d restantes</span>
                  </div>

                  <h3 style={{ fontSize:'20px', fontWeight:800, color:'var(--text-1)', letterSpacing:'-0.5px', lineHeight:1.2, marginBottom:'10px' }}>{ev.title}</h3>
                  <p style={{ fontSize:'14px', color:'var(--text-3)', lineHeight:1.6, marginBottom:'16px' }}>{ev.desc}</p>
                  ...
                  <div style={{ display:'flex', flexDirection:'column', gap:'6px', marginBottom:'16px' }}>
                    {[['📅', fmtDate(ev.date)],['🕐', ev.time + ' hrs'],['📍', ev.location],['🏘️', ev.community]].map(([ic,txt]) => (
                      <div key={txt} style={{ display:'flex', alignItems:'center', gap:'8px' }}>
                        ...
                        <span style={{ fontSize:'13px', color:'var(--text-3)' }}>{txt}</span>
                      </div>
                    ))}
                  </div>
                  ...
                  <div style={{ marginBottom:'18px' }}>
                    <div style={{ display:'flex', justifyContent:'space-between', marginBottom:'5px' }}>
                      <span style={{ fontSize:'12px', color:'var(--text-4)', fontWeight:500 }}>{ev.attendees} / {ev.cap} asistentes</span>
                      <span style={{ fontSize:'12px', color: pct>80?'#FCA87A':'var(--text-4)', fontWeight:600 }}>{pct}%</span>
                    </div>
                    <div style={{ height:'5px', borderRadius:'5px', background:'var(--surface-1)' }}>
                  ...
                  <button onClick={()=>toggleRSVP(ev.id)}
                    style={{ width:'100%', padding:'11px', borderRadius:'14px', border:'none', fontSize:'14px', fontWeight:700, cursor:'pointer', transition:'all .2s', fontFamily:'inherit',
                      background: ev.rsvp ? 'var(--surface-2)' : `linear-gradient(135deg,${ev.color},${ev.color}AA)`,
                      color:      ev.rsvp ? 'var(--text-3)' : '#fff',
        ...
        {list.filter(e=>!e.featured).length === 0 && (
          <div style={{ textAlign:'center', padding:'80px 32px' }}>
            ...
            <p style={{ fontSize:'22px', fontWeight:700, color:'var(--text-3)' }}>Sin eventos en esta categoría</p>
          </div>
        )}
      </div>
    </div>
