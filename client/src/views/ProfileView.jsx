...
export default function ProfileView() {
  ...
  const foc = e => { e.target.style.borderColor='#7C3AED'; e.target.style.boxShadow='0 0 0 4px rgba(124,58,237,0.10)'; e.target.style.background='var(--surface-2)'; };
  const blu = e => { e.target.style.borderColor='var(--divider)'; e.target.style.boxShadow='none'; e.target.style.background='var(--surface-1)'; };
    ...
    <div className="profile-bg flex">
      ...
      <aside className="sidebar-light w-[90px] fixed top-0 left-0 h-screen z-50 flex flex-col items-center py-7">
        ...
        <button onClick={toggleTheme} title={theme === 'dark' ? 'Cambiar a modo claro' : 'Cambiar a modo oscuro'}
          className="theme-toggle" style={{ marginBottom:'16px' }}>
            ...
            return (
              <button key={item.href}
                ...
                style={{
                  ...
                  color:      on ? '#7C3AED' : 'var(--text-4)',
                  boxShadow:  on ? '0 4px 16px rgba(124,58,237,0.18), inset 0 0 0 1px rgba(124,58,237,0.20)' : 'none',
                }}
                onMouseEnter={e => { if (!on) { e.currentTarget.style.background='var(--surface-hover)'; e.currentTarget.style.color='var(--text-2)'; } }}
                onMouseLeave={e => { if (!on) { e.currentTarget.style.background='transparent'; e.currentTarget.style.color='var(--text-4)'; } }}
              >
                <Ico d={item.d}/>
        ...
        <div className="w-10 h-px mb-4" style={{ background:'var(--divider)' }}/>
      ...
      <main className="ml-[90px] flex-1 px-[52px] pt-10 pb-[60px] max-w-[920px]">
            ...
            {editing ? (
              <div className="grid grid-cols-2 gap-[14px]">
                <div className="col-span-2">
                  <label className="block text-xs font-bold t4 tracking-[0.08em] uppercase mb-[7px]">Nombre completo</label>
                  <input value={form.full_name ?? ''} onChange={e => setForm(f => ({ ...f, full_name:e.target.value }))}
                    placeholder="Tu nombre completo" className="profile-input" onFocus={foc} onBlur={blu}/>
                </div>
                <div>
                  <label className="block text-xs font-bold t4 tracking-[0.08em] uppercase mb-[7px]">Universidad</label>
                ...
                <div>
                  <label className="block text-xs font-bold t4 tracking-[0.08em] uppercase mb-[7px]">Municipio de origen</label>
                ...
                <div className="col-span-2">
                  <label className="block text-xs font-bold t4 tracking-[0.08em] uppercase mb-[7px]">Biografía</label>
                ...
                <div className="col-span-2 flex gap-[10px] justify-end pt-2">
                  <button onClick={() => setEditing(false)}
                    className="px-[22px] py-[11px] rounded-[14px] bg-transparent border-[1.5px] border-[rgba(124,58,237,0.20)] t4 text-[15px] font-semibold cursor-pointer font-[inherit] transition-all">
                    Cancelar
                  </button>
              ...
              /* Modo vista */
              <div>
                <h1 className="text-[44px] font-black tracking-[-2px] t1 leading-none mb-2">
                  {profile?.full_name ?? 'Tu nombre'}
                </h1>

                <div className="flex flex-wrap gap-4 mb-[18px] items-center">
                  {profile?.university && (
                    <span className="flex items-center gap-1.5 text-[15px] t4 font-medium">
                ...
                {profile?.bio
                  ? <p className="text-[17px] t2 leading-[1.7] max-w-[600px] italic border-l-[3px] border-[rgba(124,58,237,0.30)] pl-4">
                      {profile.bio}
                    </p>
                  : <p className="text-[15px] t5 italic cursor-pointer"
          ...
          {[
            ...
            >
              ...
              <div>
                ...
                <p className="text-[13px] t4 mt-1 font-semibold">{s.l}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="card-light overflow-hidden anim-up delay-2">
          ...
          {tab === 'posts' && (
            <div className="p-7">
              {loading ? (
                <div className="text-center py-12 t4 text-base">Cargando publicaciones...</div>
              ) : posts.length === 0 ? (
                <div className="text-center px-8 py-16">
                  <span className="text-[56px] block mb-4">✍️</span>
                  <h3 className="text-[22px] font-bold t1 mb-2">Aún no has publicado</h3>
                  <p className="text-base t4 mb-6 leading-[1.6]">
                ...
                <div className="flex flex-col gap-4">
                  {posts.map((post, i) => (
                    <div key={i}
                      className="px-6 py-[22px] rounded-[20px] border transition-all duration-200 sf1 bd-divider"
                      ...
                      onMouseLeave={e => { e.currentTarget.style.background='var(--surface-1)'; e.currentTarget.style.borderColor='var(--divider)'; }}
                    >
                      <div className="flex items-center gap-[10px] mb-3">
                        ...
                        <span className="text-[13px] t4 ml-auto">
                      ...
                      <div className="flex gap-2 mt-[14px] pt-3 border-t border-[rgba(124,58,237,0.08)]">
                        {(post.reactions ?? []).length > 0
                          ...
                          : <span className="text-[13px] t5 italic">Sin reacciones aún</span>
                        }
                      </div>
                    </div>
                  ))}
            ...
            <div className="p-7">
              ...
              <div className="mt-6 px-6 py-5 rounded-[20px] border border-[rgba(124,58,237,0.14)] bg-gradient-to-r from-[rgba(124,58,237,0.08)] to-[rgba(37,99,235,0.06)]">
                <div className="flex justify-between items-center mb-[10px]">
                  <p className="text-[15px] font-bold t1">Progreso de insignias</p>
