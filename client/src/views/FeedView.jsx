...
const NAV_ITEMS = [
 ...
 export default function FeedView() {
  ...
  return (
    <div className="min-h-screen relative overflow-hidden t1">
      ...
      <div className="relative z-20 flex flex-col min-h-screen px-8 pt-6 pb-10">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-3 cursor-pointer" onClick={() => navigate('/feed')}>
            ...
            <div>
              <p className="text-[18px] font-extrabold leading-none tracking-tight t1">AfroDigital</p>
              <p className="text-xs t4 tracking-[0.05em] mt-0.5">UPB · {hoy}</p>
            </div>
          </div>
                ...
                <button key={item.href}
                  ...
                  className={`px-[18px] py-[9px] rounded-full text-sm border-none cursor-pointer transition-all duration-[250ms] font-[inherit] ${
                    on
                      ...
                      : 'bg-transparent t3 font-medium hover:t1'
                  }`}
          ...
          <div className="flex items-center gap-3">
            <button onClick={toggleTheme} title={theme === 'dark' ? 'Cambiar a modo claro' : 'Cambiar a modo oscuro'} className="theme-toggle">
              {theme === 'dark' ? <SunIcon/> : <MoonIcon/>}
            </button>
            <div className="text-right cursor-pointer" onClick={() => navigate('/profile')}>
              <p className="text-[15px] font-semibold t1 leading-none">
        ...
        <div className="grid grid-cols-[280px_1fr] gap-5 flex-1 items-start">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="g3d-strong px-6 pt-7 pb-7 text-center cursor-pointer"
              ...
              <h3 className="text-lg font-extrabold t1 mb-1 tracking-tight">{profile?.full_name}</h3>
              <p className="text-[13px] t3 mb-5">{profile?.university ?? 'UPB'}</p>

              <div className="grid grid-cols-3 gap-2">
                {[[posts.length,'Posts','#7C3AED'],[0,'Comun.','#2563EB'],[0,'Logros','#FB923C']].map(([n,l,c]) => (
                  <div key={l} className="py-3 px-2 rounded-[14px] sf1 border bd1">
                    <p className="text-2xl font-black leading-none tracking-tight" style={{ color:c }}>{n}</p>
                    <p className="text-[11px] t4 mt-0.5 font-medium">{l}</p>
                  </div>
                ))}
              </div>
              <p className="text-[11px] t5 mt-3">Toca para ver tu perfil →</p>
            </div>

            {/* Comunidades */}
            <div className="g3d p-5">
              <div className="flex justify-between items-center mb-[14px]">
                <p className="text-xs uppercase tracking-[0.14em] t4 font-semibold">Mis comunidades</p>
              ...
              {[
                ...
                <div key={com.n} onClick={() => navigate('/community')}
                  className="flex items-center gap-[10px] p-[10px] rounded-xl mb-1 cursor-pointer transition-colors hover:bg-[var(--surface-hover)]">
                  <div className="w-9 h-9 rounded-[10px] flex items-center justify-center text-[16px] flex-shrink-0"
                    style={{ background:com.c }}>
                    {com.n[0]}
                  </div>
                  <div>
                    <p className="text-sm font-semibold t2 leading-none">{com.n}</p>
                    <p className="text-xs t4 mt-0.5">{com.m}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="g3d p-5">
              <div className="flex justify-between items-center mb-[14px]">
                <p className="text-xs uppercase tracking-[0.14em] t4 font-semibold">Próximos eventos</p>
              ...
              {[
                ...
                <div key={ev.n} onClick={() => navigate('/events')}
                  ...
                  <p className="text-[13px] font-semibold t2 leading-[1.3]">{ev.n}</p>
                </div>
              ))}
            </div>
            ...
            <div className="g3d p-5">
              <p className="text-xs uppercase tracking-[0.14em] t4 font-semibold mb-[14px]">Reacciones culturales</p>
              {[
                ...
                <div key={lb}
                  className="flex items-center gap-[10px] px-3 py-[10px] rounded-xl border bd-divider mb-1.5"
                  style={{ background:bg }}>
                  <span className="text-xl">{em}</span>
                  <span className="text-sm font-medium t2">{lb}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="g3d-strong overflow-hidden relative min-h-[210px] flex items-center">
              ...
              <div className="relative z-10 px-8 py-7 max-w-[520px]">
                ...
                <p className="t3 leading-relaxed text-[15px]">El feed ahora abre con una pieza visual propia para que la experiencia se sienta más cercana a AfroDigital y menos como una red social genérica.</p>
              </div>
            </div>
            ...
            <div className="g3d-strong overflow-hidden transition-all duration-300">
              {showForm
                ? <PostForm onSuccess={onPost} onCancel={() => setShowForm(false)} />
                : (
                  <div className="px-[26px] py-[22px]">
                    <div className="flex items-center gap-[14px]">
                      ...
                      <button onClick={() => setShowForm(true)}
                        className="flex-1 text-left px-[18px] py-[14px] rounded-[14px] text-base cursor-pointer transition-all duration-200 sf1 border bd1 t4 font-[inherit]"
                        onMouseEnter={e => { e.currentTarget.style.background='var(--surface-hover)'; e.currentTarget.style.borderColor='rgba(124,58,237,0.40)'; e.currentTarget.style.color='var(--text-3)'; }}
                        onMouseLeave={e => { e.currentTarget.style.background='var(--surface-1)'; e.currentTarget.style.borderColor='var(--surface-1-border)'; e.currentTarget.style.color='var(--text-4)'; }}
                      >
                        ¿Qué quieres compartir con la comunidad hoy?
                      </button>
                    </div>

                    <div className="flex gap-2 mt-4 pt-4 border-t bd-divider">
                      {[['📷','Foto'],['🎵','Audio'],['✍️','Historia'],['📍','Lugar']].map(([ic,lb]) => (
                        <button key={lb} onClick={() => setShowForm(true)}
                          className="flex items-center gap-1.5 px-[14px] py-2 rounded-[10px] text-sm font-medium cursor-pointer transition-all duration-200 sf1 border bd1 t3 font-[inherit]"
                          ...
                          onMouseLeave={e => { e.currentTarget.style.background='var(--surface-1)'; e.currentTarget.style.borderColor='var(--surface-1-border)'; e.currentTarget.style.color='var(--text-3)'; }}
                        >{ic} {lb}</button>
                      ))}
                    </div>
                  </div>
            ...
            {posts.length > 0
              ? posts.map((post, i) => (
                <div key={i}
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      <p className="text-[15px] font-bold t1">{profile?.full_name}</p>
                      <p className="text-xs t4 mt-0.5">ahora mismo</p>
                    </div>
                    {post.category && (
                  ...
                  <p className="text-[17px] leading-[1.70] t2">{post.content}</p>

                  <div className="flex gap-2 mt-[18px] pt-4 border-t bd-divider">
                    {[['👊🏿','Apoyo'],['💜','Me representa'],['🔥','Fuego']].map(([em,lb]) => (
                      <button key={lb}
                        className="flex items-center gap-1.5 px-[14px] py-2 rounded-full text-[13px] font-medium cursor-pointer transition-all duration-200 sf1 border bd1 t3 font-[inherit]"
                        ...
                        onMouseLeave={e => { e.currentTarget.style.background='var(--surface-1)'; e.currentTarget.style.color='var(--text-3)'; e.currentTarget.style.borderColor='var(--surface-1-border)'; }}
                      >{em} {lb}</button>
                    ))}
                  </div>
                </div>
              ...
              : (
                <div className="g3d px-8 py-[72px] text-center">
                  ...
                  <h3 className="text-[26px] font-extrabold t2 mb-[10px] tracking-tight">
                    El feed espera sus primeras voces
                  </h3>
                  <p className="text-[17px] t4 max-w-[320px] mx-auto mb-7 leading-[1.65]">
