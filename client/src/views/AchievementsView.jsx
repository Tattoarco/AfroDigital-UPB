...
const LEADERBOARD = [
  ...
  { name:'Carlos Angulo',    xp:720,  rank:4, color:'var(--text-4)' },
  { name:'Tú',               xp:50,   rank:8, color:'#FCA87A', isMe:true },
];

const G = {
  background:'var(--surface-1)', backdropFilter:'blur(40px)', WebkitBackdropFilter:'blur(40px)',
  border:'1px solid var(--surface-1-border)', borderTop:'1px solid var(--surface-1-border-top)',
...
export default function AchievementsView() {
  ...
  return (
    <div style={{ minHeight:'100vh', position:'relative', color:'var(--text-1)' }}>
      ...
      {/* FONDO */}
      <div style={{ position:'fixed', inset:0, zIndex:0, background:'var(--bg-achievements)' }}/>
      ...
      <div style={{ marginLeft:'90px', padding:'40px 48px 60px', position:'relative', zIndex:1, maxWidth:'1100px' }}>
        ...
        <div style={{ marginBottom:'36px', display:'flex', alignItems:'center', justifyContent:'space-between', gap:'28px' }}>
          <div>
          <p style={{ fontSize:'12px', letterSpacing:'0.16em', textTransform:'uppercase', color:'var(--text-4)', fontWeight:600, marginBottom:'6px' }}>AfroDigital UPB</p>
          <h1 style={{ fontSize:'64px', fontWeight:900, lineHeight:0.92, letterSpacing:'-3px' }}>
            ...
            <span style={{ color:'var(--text-1)' }}>ros</span>
          </h1>
          </div>
          <img src={badgeTotem} alt="Tótem visual de insignias AfroDigital" style={{ width:'180px', height:'180px', objectFit:'cover', borderRadius:'32px', border:'1px solid var(--surface-1-border)', boxShadow:'0 24px 72px rgba(0,0,0,0.38)' }} />
        </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:'16px', marginBottom:'28px' }}>
          ...
          <div style={{ ...G, borderRadius:'24px', padding:'28px', position:'relative', overflow:'hidden', gridColumn:'span 2' }}>
            ...
            <div style={{ position:'relative', zIndex:1 }}>
              <p style={{ fontSize:'13px', letterSpacing:'0.12em', textTransform:'uppercase', color:'var(--text-4)', fontWeight:600, marginBottom:'6px' }}>Puntos XP</p>
              ...
              <p style={{ fontSize:'15px', color:'var(--text-4)', marginTop:'4px' }}>de {maxXP} XP posibles</p>

              {/* Barra de XP */}
              <div style={{ marginTop:'20px' }}>
                <div style={{ display:'flex', justifyContent:'space-between', marginBottom:'8px' }}>
                  <span style={{ fontSize:'13px', color:'var(--text-4)' }}>Progreso total</span>
                  <span style={{ fontSize:'13px', color:'#FCA87A', fontWeight:700 }}>{pct}%</span>
                </div>
                <div style={{ height:'10px', borderRadius:'10px', background:'var(--surface-1)' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
            {[
              ...
              { n:ALL_BADGES.length - earned.length, l:'Bloqueadas', c:'var(--text-5)' },
              ...
              <div key={s.l} style={{ ...G, borderRadius:'16px', padding:'16px 18px', flex:1, display:'flex', alignItems:'center', gap:'12px' }}>
                ...
                <p style={{ fontSize:'13px', color:'var(--text-4)', fontWeight:500 }}>{s.l}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          {[['badges','🏆 Insignias'], ['challenges','⚡ Retos activos'], ['leaderboard','🌟 Tabla de honor']].map(([k,l]) => (
            <button key={k} onClick={()=>setTab(k)}
              style={{ padding:'10px 20px', borderRadius:'12px', fontSize:'14px', fontWeight:600, cursor:'pointer', transition:'all .2s', border:'1px solid', fontFamily:'inherit',
                background: tab===k ? 'rgba(124,58,237,0.22)' : 'var(--surface-1)',
                color:      tab===k ? '#A78BFA'                : 'var(--text-3)',
                borderColor:tab===k ? 'rgba(124,58,237,0.45)' : 'var(--surface-1-border)',
              }}
            >{l}</button>
          ))}
        </div>
          ...
          <div>
            ...
            {earned.length > 0 && (
              <div style={{ marginBottom:'28px' }}>
                <p style={{ fontSize:'13px', letterSpacing:'0.12em', textTransform:'uppercase', color:'var(--text-4)', fontWeight:600, marginBottom:'14px' }}>Obtenidas · {earned.length}</p>
                  ...
                  {earned.map(b => (
                    ...
                    >
                      ...
                      <div style={{ position:'relative', zIndex:1 }}>
                        ...
                        <h4 style={{ fontSize:'17px', fontWeight:800, color:'var(--text-1)', marginBottom:'5px', letterSpacing:'-0.3px' }}>{b.name}</h4>
                        <p style={{ fontSize:'13px', color:'var(--text-3)', lineHeight:1.5 }}>{b.desc}</p>
            ...
            {/* Bloqueadas */}
            <p style={{ fontSize:'13px', letterSpacing:'0.12em', textTransform:'uppercase', color:'var(--text-5)', fontWeight:600, marginBottom:'14px' }}>Por desbloquear · {locked.length}</p>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(220px,1fr))', gap:'14px' }}>
              {locked.map(b => (
                ...
                >
                  ...
                  <h4 style={{ fontSize:'17px', fontWeight:800, color:'var(--text-2)', marginBottom:'5px' }}>{b.name}</h4>
                  <p style={{ fontSize:'13px', color:'var(--text-4)', lineHeight:1.5, marginBottom:'10px' }}>{b.desc}</p>
                  <p style={{ fontSize:'12px', color:'var(--text-4)', fontStyle:'italic' }}>→ {b.how}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {tab === 'challenges' && (
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))', gap:'16px' }}>
            {CHALLENGES.map((ch, i) => {
              ...
              return (
                <div key={i} style={{ ...G, borderRadius:'22px', padding:'26px', border:done?`1px solid ${ch.color}40`:'1px solid var(--surface-1-border)', borderTop:done?`1px solid ${ch.color}60`:'1px solid var(--surface-1-border-top)', transition:'all .3s' }}
                ...
                >
                  ...
                  <h4 style={{ fontSize:'18px', fontWeight:800, color:done?ch.color:'var(--text-1)', marginBottom:'6px' }}>{ch.title}</h4>
                  <p style={{ fontSize:'14px', color:'var(--text-3)', lineHeight:1.5, marginBottom:'18px' }}>{ch.desc}</p>

                  <div style={{ marginBottom:'10px' }}>
                    <div style={{ display:'flex', justifyContent:'space-between', marginBottom:'6px' }}>
                      <span style={{ fontSize:'12px', color:'var(--text-4)' }}>{ch.progress} / {ch.total}</span>
                      <span style={{ fontSize:'12px', color:done?ch.color:'var(--text-4)', fontWeight:600 }}>{done?'✓ Completado':pct+'%'}</span>
                    </div>
                    <div style={{ height:'8px', borderRadius:'8px', background:'var(--surface-1)' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
              {LEADERBOARD.map((u, i) => (
                <div key={i} style={{ ...G, borderRadius:'18px', padding:'18px 22px', display:'flex', alignItems:'center', gap:'16px',
                  background: u.isMe ? 'rgba(124,58,237,0.15)' : 'var(--surface-1)',
                  border: u.isMe ? '1px solid rgba(124,58,237,0.35)' : '1px solid var(--surface-1-border)',
                  transition:'all .3s' }}
                ...
                >
                  <div style={{ width:'36px', textAlign:'center' }}>
                    {u.rank <= 3
                      ...
                      : <span style={{ fontSize:'18px', fontWeight:700, color:'var(--text-4)' }}>#{u.rank}</span>
                    }
                  </div>
                  ...
                  <div style={{ flex:1 }}>
                    <p style={{ fontSize:'16px', fontWeight:700, color: u.isMe?'#A78BFA':'var(--text-1)', lineHeight:1 }}>
                      {u.name} {u.isMe && <span style={{ fontSize:'12px', fontWeight:500, color:'#A78BFA', marginLeft:'6px' }}>← tú</span>}
                    </p>
                  </div>
                  <div style={{ textAlign:'right' }}>
                    <p style={{ fontSize:'22px', fontWeight:900, color:u.rank<=3?['#FB923C','rgba(167,139,250,0.80)','rgba(96,165,250,0.80)'][u.rank-1]:'var(--text-3)', lineHeight:1, letterSpacing:'-1px' }}>{u.xp}</p>
                    <p style={{ fontSize:'11px', color:'var(--text-4)', marginTop:'2px' }}>XP</p>
                  </div>
                </div>
              ))}
            </div>
            <p style={{ fontSize:'13px', color:'var(--text-5)', marginTop:'20px', textAlign:'center', fontStyle:'italic' }}>
